...
main (void) {
  ...
  // normal for address sanitizer to complain about this, we have our custom
  // printf specifier :)
  printf ("sv.str=%v\n", sv);
  ...
  // test iterator
  for (struct json_obj *cur = json_begin (&json_parsed);
       cur != json_end (&json_parsed); cur = json_next (&json_parsed, cur)) {
    if (cur == NULL) {
  ...
  // get a value by key
  struct json_obj *obj1 = json_get (&json_parsed, "build_date");
