...
int main () {
  ...
  FILE *fp = fopen ("/home/zichuan/json-c/test-cases/test1.json", "r");
	...
	// test iterator
	for (
	  struct json_obj *cur = json_begin (&json1);
	  cur != json_end (&json1);
	...
	struct json_obj *obj = json_get_by_key (&json1, "build_date");

	str_view_t sv = obj->value.str;

	printf ("[build_date]: %.*s\n", (int) sv.len, sv.str);

	struct json_obj *obj1 = json_get_by_index (&json1, 2);
	if (obj1 == NULL) {
		printf ("error\n");
		return -1;
	}
